  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      const filePath = await save({
        defaultPath: `${agent.name.toLowerCase().replace(/\s+/g, '-')}.codestudio.json`,
        filters: [{
          name: 'CodeStudio Agent',
          extensions: ['codestudio.json']
        }]
      });
  ...
  const handleImportAgent = async () => {
    try {
      ...
      const filePath = await open({
        ...
        filters: [{
          name: 'CodeStudio Agent',
          extensions: ['codestudio.json', 'json']
        }]
      });
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              ...
                              <CardFooter className="p-4 pt-0 flex justify-center gap-1 flex-wrap">
                                ...
                                <Button
                                  ...
                                  className="flex items-center gap-1"
                                  title="Export agent to .codestudio.json"
                                >
